...
async def cb_manage(update: Update, ctx):
    ...
    if d == "st_work_mode":
        ...
        if work_on:
            status = "🟢 *وضع العمل مفعّل*"
            desc   = ("أي تعديل تجريه الآن على الأزرار أو الأسماء أو المحتوى\n"
                      "*لا يظهر للمستخدمين* — حتى تضغط Push.\n\n"
                      "حتى لو أُعيد تشغيل البوت، التغييرات محفوظة ووضع العمل يبقى مفعّلاً.\n\n"
                      "• ⬆️ *Push* — ينشر كل تعديلاتك للمستخدمين\n"
                      "• ❌ *إلغاء* — يتراجع عن كل التعديلات ويعيد ما كان قبل العمل")
        else:
            status = "⭕ *وضع العمل غير مفعّل*"
            desc   = ("عند التفعيل، أي تعديل تجريه على الأزرار أو الأسماء أو المحتوى\n"
                      "*لن يظهر للمستخدمين* حتى تضغط ⬆️ Push.\n\n"
                      "حتى لو أُعيد تشغيل البوت وأنت في وضع العمل،\n"
                      "التغييرات تبقى محفوظة ووضع العمل يبقى نشطاً.")
        await q.edit_message_text(
            f"🔧 *وضع العمل*\n\n{status}\n\n{desc}",
            parse_mode="Markdown",
    ...
    if d == "st_work_start":
        ...
        await q.edit_message_text(
            "🟢 *تم تفعيل وضع العمل*\n\n"
            "المستخدمون يرون النسخة الحالية المجمّدة.\n"
            "أجرِ تعديلاتك براحة — حتى لو أُعيد تشغيل البوت كل شيء محفوظ.\n\n"
            "اضغط ⬆️ *Push* عند الانتهاء لنشر التغييرات.",
            parse_mode="Markdown",
            reply_markup=kb_work_mode()
        )
        return
    ...
    if d == "st_work_cancel":
        ...
        await q.edit_message_text(
            "❌ *تم إلغاء وضع العمل*\n\n"
            "تم التراجع عن جميع التغييرات واستعادة النسخة كما كانت.",
            parse_mode="Markdown",
            reply_markup=kb_work_mode()
        )
        return
